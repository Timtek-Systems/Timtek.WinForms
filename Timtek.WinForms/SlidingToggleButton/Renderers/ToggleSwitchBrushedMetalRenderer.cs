...
public class ToggleSwitchBrushedMetalRenderer : ToggleSwitchRendererBase
{
    ...
    public override void RenderBorder(Graphics g, Rectangle borderRectangle)
    {
        ...
        using (var outerControlPath = GetRectangleClipPath(borderRectangle))
        {
            ...
            var borderColor1 = !ToggleSwitch.Enabled && ToggleSwitch.GrayWhenDisabled ? BorderColor1.ToGrayScale() : BorderColor1;
            var borderColor2 = !ToggleSwitch.Enabled && ToggleSwitch.GrayWhenDisabled ? BorderColor2.ToGrayScale() : BorderColor2;
        ...
        var innercontrolRectangle = new Rectangle(borderRectangle.X + 1, borderRectangle.Y + 1, borderRectangle.Width - 1,
        ...
        using (var innerControlPath = GetRectangleClipPath(innercontrolRectangle))
        {
            ...
            using (Brush backgroundBrush =
                   new LinearGradientBrush(borderRectangle, backColor1, backColor2, LinearGradientMode.Horizontal))
            {
                g.FillPath(backgroundBrush, innerControlPath);
            }

            //Draw inner top shadow
            var upperShadowRectangle = new Rectangle(innercontrolRectangle.X, innercontrolRectangle.Y, innercontrolRectangle.Width,
                UpperShadowHeight);

            g.IntersectClip(upperShadowRectangle);

            using (Brush shadowBrush = new LinearGradientBrush(upperShadowRectangle, UpperShadowColor1, UpperShadowColor2,
    ...
    public override void RenderLeftToggleField(Graphics g, Rectangle leftRectangle, int totalToggleFieldWidth)
    {
        ...
        var innercontrolRectangle = new Rectangle(1, 1, ToggleSwitch.Width - 1, ToggleSwitch.Height - 2);
    ...
    public override void RenderButton(Graphics g, Rectangle buttonRectangle)
    {
        ...
        //Draw button surface
        var buttonSurfaceColor = ButtonNormalSurfaceColor;
        ...
        //Draw "metal" surface
        var centerPoint1 = new PointF(buttonRectangle.X + buttonRectangle.Width / 2f,
            buttonRectangle.Y + 1.2f * (buttonRectangle.Height / 2f));

        using (var firstMetalBrush = GetBrush(new[]
               {
                   Color.Transparent, Color.Transparent, Color.Transparent, Color.Transparent, Color.Transparent, Color.Transparent,
                   Color.Transparent, Color.Transparent, Color.Transparent, Color.FromArgb(255, 110, 110, 110), Color.Transparent,
                   Color.Transparent,
                   Color.Transparent
        ...
        var centerPoint2 = new PointF(buttonRectangle.X + 0.8f * (buttonRectangle.Width / 2f),
            buttonRectangle.Y + buttonRectangle.Height / 2f);

        using (var secondMetalBrush = GetBrush(new[]
               {
        ...
        var centerPoint3 = new PointF(buttonRectangle.X + 1.2f * (buttonRectangle.Width / 2f),
            buttonRectangle.Y + buttonRectangle.Height / 2f);

        using (var thirdMetalBrush = GetBrush(new[]
               {
                   Color.Transparent, Color.Transparent, Color.Transparent, Color.Transparent,
        ...
        var centerPoint4 = new PointF(buttonRectangle.X + 0.9f * (buttonRectangle.Width / 2f),
            buttonRectangle.Y + 0.9f * (buttonRectangle.Height / 2f));

        using (var fourthMetalBrush = GetBrush(new[]
               {
                   Color.Transparent, Color.FromArgb(255, 188, 188, 188), Color.FromArgb(255, 110, 110, 110), Color.Transparent,
        ...
        //Draw button border
        var buttonBorderColor = ButtonNormalBorderColor;
    ...
    private PathGradientBrush GetBrush(Color[] Colors, RectangleF r, PointF centerPoint)
    {
        ...
        var w = (int)Math.Floor(180.0 * (i - 2.0) / i / 2.0);
        var wi = w * Math.PI / 180.0;
        var faktor = 1.0 / Math.Sin(wi);

        var radx = (float)(cx * faktor);
        var rady = (float)(cy * faktor);
        ...
        fBrush.SurroundColors = new[] { Color.White };
